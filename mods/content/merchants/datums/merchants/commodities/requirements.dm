...
/**
Acts as a condition that will determine whether or not `instance` qualifies as something the merchant would want to buy.
Called whenever a player tries to sell to a merchant. All `/decl/merchant_commodity_requirement`s must return TRUE for the merchant to buy something.
...
- `atom/movable/instance`: The instance that is being evaluated.
*/
/*
/decl/merchant_commodity_requirement/proc/evaluate(list/parameters, atom/movable/instance)
	return FALSE
*/
...
/decl/merchant_commodity_requirement/type/anything/generate_name(list/parameters, list/all_requirements)
	return "anything"
/*
/decl/merchant_commodity_requirement/type/anything/evaluate(list/parameters, atom/movable/instance)
	if(!istype(instance))
		return FALSE // This shouldn't ever happen, but...
	return TRUE
*/
...
/*
/// Require that an object be of a particular type. Allows for including and excluding various types, whether related or not.
/decl/merchant_commodity_requirement/typepath

/// Converts an assoc list of type paths and instructions into a typecache-like format for future use.
/decl/merchant_commodity_requirement/typepath/build_parameters(list/parameters)
	var/list/types_included = list()
	for(var/type_path, instruction in parameters)
	...
	// Convert to the typecache format, so comparisons can be quicker.
	var/list/final_typecache = list()
	for(var/type_path in types_included)
		final_typecache[type_path] = TRUE
	return final_typecache


/decl/merchant_commodity_requirement/typepath/generate_name(list/parameters, list/all_requirements)
	var/list/atom_names = list()
	for(var/type_path in parameters)
		var/atom_name = atom_info_repository.get_name_for(type_path)
		atom_names[atom_name] = TRUE // So duplicates don't get repeated multiple times.

	return english_list(atom_names, and_text = " or ")

/decl/merchant_commodity_requirement/typepath/evaluate(list/parameters, atom/movable/instance)
	return is_type_in_typecache(instance, parameters)
*/

/decl/merchant_commodity_requirement/mobs

/// Require that a mob be of a particular species.
...
/decl/merchant_commodity_requirement/material
	naming_position = -1 // Prefix.
/*
/decl/merchant_commodity_requirement/material/generate_name(parameter, list/all_requirements)
	var/decl/material/material = GET_DECL(parameter)
	return material.adjective_name
*/
...
/decl/merchant_commodity_requirement/gas/ratio/generate_name(list/parameters, list/all_requirements)
	...
	//	text += "[length(parameters) == 1 ? "only" : "[ratio * 100]%"] [material.gas_name]"
		text += "[length(parameters) == 1 ? "" : "[ratio * 100]% "][material.gas_name]"
	return "containing [english_list(text)]"
...
/decl/merchant_commodity_requirement/reagents/ratio/compare_instance(list/parameters, atom/movable/instance)
	...
	for(var/thing, volume in reagents.reagent_volumes)
		var/decl/material/reagent = thing
		if(!parameters[reagent.type])
			return FALSE // Unwanted reagent.
		var/actual_ratio = volume / reagents.total_volume
...
/decl/merchant_commodity_requirement/reagents/has_reagents
	naming_position = -1
...
//	return "containing [english_list(reagent_names, and_text = " or ")]"
	return english_list(reagent_names, and_text = " or ")
...
/decl/merchant_commodity_requirement/reagents/has_reagents/compare_instance(list/parameters, atom/movable/instance)
	...
	for(var/thing in reagents.reagent_volumes)
		var/decl/material/reagent = thing
