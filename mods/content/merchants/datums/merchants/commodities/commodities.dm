...
//	to_world_log("/datum/merchant_commodity/New(): _requirements: [json_encode(_requirements)]")
	for(var/type_path, parameters in _requirements)
//		to_world_log("type_path: [type_path], parameters = [json_encode(parameters)]")
...
/datum/merchant_commodity/proc/check_requirements(atom/movable/thing)
	for(var/type_path, parameters in requirements)
		...
		if(!requirement.compare_instance(parameters, thing))
			return FALSE
//		if(!requirement.evaluate(parameters, instance))
//			to_world("Demand ([name]): [instance] failed requirement [requirement.type]")
//			return FALSE
	return TRUE
