...
//	if(length(types_included) > max_distinct_types)
//		while(length(types_included) > max_distinct_types)
//			pick_n_take(types_included)
...
//		var/list/requirements = list(/decl/merchant_commodity_requirement/type/exact_type = type_path)
...
/decl/merchant_potential_commodities/material_stacks/all_mineable_ores/New()
...
/*
	...
	for(var/container_type in container_types)
		for(var/gas_type in gas_types)
			...
			requirements[/decl/merchant_commodity_requirement/gas/ratio] = list( (gas_type) = 1.0) // TODO: Find way to add gas mixes, most notably Air.
...
/*
/decl/merchant_potential_commodities/almost_any_item
	var/minimum_types = 10
	var/maximum_types = 20
	var/list/prohibited_types = list(
		/obj/item/projectile			= MERCHANT_INCLUDE_ALL,
		/obj/item/assembly				= MERCHANT_EXCLUDE_ALL,
		/obj/item/assembly_holder		= MERCHANT_EXCLUDE_ALL,
		/obj/item/encryptionkey/hacked	= MERCHANT_EXCLUDE_ALL,
		/obj/item/tank/onetankbomb		= MERCHANT_EXCLUDE_ALL,
		/obj/item/radio					= MERCHANT_EXCLUDE_ALL,
		/obj/item/modular_computer/pda 	= MERCHANT_EXCLUDE_SUBTYPES,
		/obj/item/uplink				= MERCHANT_EXCLUDE_ALL
	)

/decl/merchant_potential_commodities/almost_any_item/create_datums()
	. = list()
	var/number_of_types = rand(minimum_types, maximum_types)
	var/list/prohibited_typecache = filter_type_paths(prohibited_types)
	var/list/chosen_types = list()
	var/list/item_types = subtypesof(/obj/item)

	for(var/i = 1 to number_of_types)
		var/index = rand(1, length(item_types))
		var/chosen_type = item_types[index]
		if(prohibited_typecache[chosen_type])
			continue
		if(is_path_in_list(chosen_type, chosen_types))
			continue
		chosen_types += chosen_type

	for(var/type in chosen_types)
		var/quantity = INFINITY
		if(item_quantity_upper_bound != INFINITY)
			quantity = rand(item_quantity_lower_bound, item_quantity_upper_bound)
		. += new /datum/merchant_commodity(list(/decl/merchant_commodity_requirement/type/exact_type = type), _item_quantity = quantity)
*/
/*
/decl/merchant_potential_commodities/semi_random_junk
	var/minimum_types = 10
	var/maximum_types = 20
	var/list/random_spawner_types = list(
		/obj/random/powercell
	)

/decl/merchant_potential_commodities/semi_random_junk/create_datums()
	. = list()
	var/number_of_types = rand(minimum_types, maximum_types)
	var/list/chosen_types = list()

	for(var/i = 1 to number_of_types)
		var/obj/random/random_spawner = pick(random_spawner_types)
		var/list/choices = call()()
//		var/list/choices = random_spawner.spawn_choices()
//		random_spawner = new()
//		var/list/choices = random_spawner.spawn_choices()
		var/chosen_type = pick(choices)
		if(!is_path_in_list(chosen_type, chosen_types))
			chosen_types += chosen_type
