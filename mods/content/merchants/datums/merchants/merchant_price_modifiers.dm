...
/decl/merchant_price_modifier/percentage/calculate(value, input, mob/user, datum/merchant/merchant)
	return value * input
/*
/decl/merchant_price_modifier/percentage/print_amount(input, mob/user, datum/merchant/merchant)
	return "[(input - 1.0)*100]%"
*/
...
/decl/merchant_price_modifier/disposition/calculate(value, input, mob/user, datum/merchant/merchant)
	var/weight = merchant.disposition / (merchant.MAX_DISPOSITION_THRESHOLD)
...
/decl/merchant_price_modifier/legacy_skill_disparity/proc/skill_curve(disparity)
	...
	if(disparity >= 0)
		// The equation simplfies to `1+x^2`,
		// resulting in: 1, 2, 5, 10, 17
		return 1 + (disparity) ** 2

	// The equation simplifies to `0.8^-x`,
	// resulting in: 0.8, 0.64, 0.51, 0.4
