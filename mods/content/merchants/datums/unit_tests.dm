...
/datum/unit_test/merchant/disposition_price_change/start_test()
	...
	my_merchant.disposition = /datum/merchant::MAX_DISPOSITION_THRESHOLD
	...
	var/failures = 0
	to_world_log("Buying price: [buying_price] Selling price: [selling_price].")
	...
	my_merchant.disposition = -my_merchant.disposition
...
/*
/datum/unit_test/merchant/skill_price_change
	name = "MERCHANTS: Merchants shall modify prices according to relative skill"

/datum/unit_test/merchant/skill_price_change/start_test()
	my_merchant.supply_price_modifiers = list(/decl/merchant_price_modifier/legacy_skill_disparity/supply = 1.2)
	my_merchant.demand_price_modifiers = list(/decl/merchant_price_modifier/legacy_skill_disparity/demand = 1.2)

	for(var/merchant_skill in SKILL_MIN to SKILL_MAX)
		my_merchant.skill_level = merchant_skill
		for(var/user_skill in SKILL_MIN to SKILL_MAX)
			fake_user.skillset.skill_list[SKILL_FINANCE] = user_skill
			to_world_log("Merchant skill: [merchant_skill], user skill: [user_skill].")
			var/selling_price = my_merchant.get_item_value(/obj/test_product/one, /datum/merchant::TRANSACTION_SELLING, fake_user)
			var/buying_price = my_merchant.get_item_value(/obj/test_product/two, /datum/merchant::TRANSACTION_BUYING, fake_user)
			to_world_log("Selling price: [selling_price] Buying price:[buying_price]")
...
/datum/unit_test/merchant/dispositon_changes_when_buying_and_selling/start_test()
	...
	if(my_merchant.disposition <= 0)
		fail("Merchant disposition did not increase after selling a product. It was [my_merchant.disposition].")

	my_merchant.disposition = 0
	my_merchant.transaction_disposition_shift(list(/obj/test_product/two), /datum/merchant::TRANSACTION_BUYING, fake_user)
	if(my_merchant.disposition <= 0)
		fail("Merchant disposition did not increase after buying a product. It was [my_merchant.disposition].")
...
/datum/unit_test/merchant/no_disposition_change_if_buying_and_selling_same_item_for_same_price/start_test()
	...
	if(my_merchant.disposition != 0)
		fail("Merchant disposition increased after buying and selling the same item for the same price. The disposition was [my_merchant.disposition].")
	else
...
/datum/unit_test/merchant/reduced_disposition_change_if_buying_and_selling_same_item_for_different_prices/start_test()
	my_merchant.transaction_disposition_shift(list(/obj/test_product/one), /datum/merchant::TRANSACTION_SELLING, fake_user)
	my_merchant.transaction_disposition_shift(list(/obj/test_product/one), /datum/merchant::TRANSACTION_BUYING, fake_user)
	var/normal_disposition = my_merchant.disposition
	...
	if(my_merchant.disposition == normal_disposition)
		fail("Merchant disposition did not increase after buying and selling the same item for different prices. The disposition was [my_merchant.disposition].")
	else
