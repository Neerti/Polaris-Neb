...
/*
/datum/computer_file/program/merchant/proc/get_current_hub(datum/trade_hub/supplied_hub)
	if(supplied_hub)
...
//	if(QDELETED(hub) || QDELETED(current_merchant) || (current_merchant && !(current_merchant in hub.merchants)))
//		current_merchant = null
...
/datum/computer_file/program/merchant/proc/insult_merchant(mob/user, datum/merchant/target_merchant)
	...
	var/decl/merchant_speech/speech = GET_DECL(target_merchant.speech)
	if(target_merchant.disposition >= target_merchant.HIGH_DISPOSITION_THRESHOLD)
...
/datum/computer_file/program/merchant/proc/bribe_merchant(mob/user, datum/merchant/target_merchant, amount)
	...
	var/decl/merchant_speech/speech = GET_DECL(target_merchant.speech)
	var/success = target_merchant.bribe(amount)
	if(success)
...
//	var/item_type = target_merchant.active_supply_by_typepath[index]
...
//	price = floor(price / target_merchant.currency_used.absolute_value) // This will result in a price of 0 if the price is 1 with a nonstandard abs value currency.
	...
	/*
	if(LAZYLEN(target_merchant.supply_fee_modifiers))
		var/total = target_merchant.get_taxes(price, target_merchant.TRANSACTION_SELLING, user)
		if(total != price)
			response = "[response]<br><br>\
			SYSTEM:<br>\
			Subtotal: [target_merchant.currency_used.format_value(price)]<br>\
			[target_merchant.print_price_modifiers(target_merchant.supply_fee_modifiers, user)]<br>\
			Total: [target_merchant.currency_used.format_value(total)]"
	*/
	set_last_comms(user, target_merchant, response)
	return TRUE
...
//	var/item_type = target_merchant.active_supply_by_typepath[index]
//	var/datum/merchant_stock_metadata/supply = target_merchant.active_supply_by_typepath[item_type]
	...
	var/merchant_price = target_merchant.get_item_value(item_type, target_merchant.TRANSACTION_SELLING, user) // TODO: Maybe optimize this by passing in the commodity datum to avoid extra work later for getting price variance?
	if(!offered_price)
...
//		value = target_merchant.get_taxes(value, target_merchant.TRANSACTION_BUYING, user)
...
//		var/datum/merchant_stock_metadata/demand = target_merchant.active_demand_by_typepath[thing.type]
...
/datum/computer_file/program/merchant/proc/attempt_gift(mob/user, datum/merchant/target_merchant)
	...
	var/list/items_on_pad = actual_pad.get_targets()
	if(!check_items_on_pad(items_on_pad, user, target_merchant))
		return FALSE // They don't want something, even for free.
...
/datum/computer_file/program/merchant/proc/check_items_on_pad(list/items, mob/user, datum/merchant/target_merchant)
	...
	/*
	// Consolidates everything inside of `items` into a new assoc list, with the type path as the key, and the quantity as the value.
	var/list/consolidated_items = list()
	for(var/atom/movable/item in items)
		if(!(consolidated_items[item.type]))
			consolidated_items[item.type] = 0
		consolidated_items[item.type] += 1

	for(var/typepath, count in consolidated_items)
		var/datum/merchant_stock_metadata/demand = target_merchant.active_demand_by_typepath[typepath]
		if(demand.quantity != INFINITY)
			if(demand.quantity <= 0)
				var/response = speech.no_more_wanted
				response = replacetext(response, /decl/merchant_speech::MERCHANT_TOKEN_ITEM, atom_info_repository.get_name_for(typepath))
				set_last_comms(user, target_merchant, response)
				return FALSE // Merchant doesn't want anymore clams.
			...
			if(count > demand.quantity)
				var/response = speech.too_many_items
				response = replacetext(response, /decl/merchant_speech::MERCHANT_TOKEN_ITEM, atom_info_repository.get_name_for(typepath))
				response = replacetext(response, /decl/merchant_speech::MERCHANT_TOKEN_QUANTITY, demand.quantity)
				set_last_comms(user, target_merchant, response)
				return FALSE // Merchant doesn't want to buy a thousand clams, just a few.
	*/
...
/datum/computer_file/program/merchant/proc/ask_what_they_want(mob/user, datum/merchant/target_merchant)
	...
	for(var/key, value in target_merchant.active_demand_by_typepath)
		var/datum/merchant_stock_metadata/demand = value
		var/item_name = atom_info_repository.get_name_for(key)
		if(demand.quantity != INFINITY)
			if(demand.quantity <= 0)
				continue // So the merchant doesn't say they want 0 of something.
			item_name = "[demand.quantity] [item_name]"
		item_names += item_name
	response += " [english_list(item_names)][speech.what_wanted_ending || "."]"
	*/
	set_last_comms(user, target_merchant, response)
	return TRUE
...
//		return stored_cash[currency_type]
...
//			stored_cash[money.currency] += money.get_worth()
			qdel(money)
			. = TRUE
	if(.)
...
//		var/decl/currency/currency = GET_DECL(currency_type)
//		dollarydoos.adjust_worth(stored_cash[currency_type] * currency.absolute_value)
	...
	if(program)
		...
		if(current_merchant)
			...
			if(P.show_items_for_sale)
				...
				/*
				if(length(current_merchant.active_supply_by_typepath))
					for(var/type_path, thing in current_merchant.active_supply_by_typepath)
						var/datum/merchant_stock_metadata/supply = thing
						var/displayed_name = P.print_item_name(type_path)
						var/displayed_quantity = null
						if(supply.quantity != INFINITY)
							if(supply.quantity <= 0)
								displayed_quantity = "SOLD OUT"
							else
								displayed_quantity = "[supply.quantity] left"
						if(!merchant_is_understood)
							displayed_name = current_merchant.scramble_response(user, displayed_name)
							displayed_quantity = current_merchant.scramble_response(user, displayed_quantity)
						item_data += list(list("item_name" = displayed_name, "item_quantity" = displayed_quantity))
...
/datum/computer_file/program/merchant/Topic(href, href_list)
	...
	if(current_merchant)
		...
		if(href_list["bribe"])
			. = TRUE
			bribe_merchant(user, current_merchant, text2num(href_list["bribe"]))
