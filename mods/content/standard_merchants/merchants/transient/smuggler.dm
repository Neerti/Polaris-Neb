/// A sneaky 'merchant' who requires a bribe for be tradeable.
...
/datum/merchant/transient/smuggler/pre_hail(mob/user)
	. = ..()
	// It is extremely unlikely for players to have somehow raised disposition high enough for this to trigger without bribes,
	// but perhaps there might be other ways to raise it in the future.
	if(disposition >= HIGH_DISPOSITION_THRESHOLD && secretive)
		reveal_secret()
...
/decl/merchant_potential_commodities/reagents/malpractice_drug_demand
	type_instructions = list(
		...
		/decl/material/liquid/antidepressants	= MERCHANT_INCLUDE_THIS_TYPE,
		/decl/material/liquid/stimulants		= MERCHANT_INCLUDE_THIS_TYPE,
