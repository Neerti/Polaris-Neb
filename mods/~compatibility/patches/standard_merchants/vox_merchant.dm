...
/datum/merchant/transient/vox/pre_hail(mob/user)
	...
	if(species?.uid == /decl/species/vox::uid && !has_met_another_vox)
		...
		set_disposition(MAX_DISPOSITION_THRESHOLD) // This used to be 1000 but it shouldn't really matter.
...
/*
/decl/merchant_potential_commodities/vox_merchant_shiny_stacks
	var/list/wanted_materials = list(
...
/decl/merchant_potential_commodities/vox_merchant_shiny_stacks/create_datums()
	. = list()
	for(var/type_path in wanted_materials)
		. += new /datum/merchant_commodity(list(
			/decl/merchant_commodity_requirement/type/type_or_subtype = /obj/item/stack/material,
			/decl/merchant_commodity_requirement/material = type_path
		))
*/
...
/decl/merchant_speech/clothing_shop/New()
	. = ..()
